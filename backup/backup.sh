...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/MyMasWayVPN/acces/main/punten > /root/tmp
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/MyMasWayVPN/acces/main/punten | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/MyMasWayVPN/acces/main/punten | grep $MYIP | awk '{print $2}')
...
cp -r /var/lib/mwvpn-pro/ /root/backup/ssnvpn-pro &> /dev/null
...
LLatest=`date`
Get_Data () {
git clone https://github.com/mymaswayvpn/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "mas120604way@gmail.com" &> /dev/null
    git config --global user.name "mymaswayvpn" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M ipuk &> /dev/null
    git remote add origin https://github.com/mymaswayvpn/backup
    git push -f https://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@github.com/mymaswayvpn/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/mymaswayvpn/backup/main/$NameUser/$NameUser.zip"
sleep 1
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • MasWaySTORE •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo
