...
APIGIT=$(cat /etc/mwstore/github/api)
EMAILGIT=$(cat /etc/mwstore/github/email)
USERGIT=$(cat /etc/mwstore/github/username)
...
if [[ -f /etc/mwstore/github/api && -f /etc/mwstore/github/email && /etc/mwstore/github/username ]]; then
   rec="OK"
else
    mkdir /etc/mwstore/github > /dev/null 2>&1
fi
...
sleep 2
echo "$EMAIL1" > /etc/mwstore/github/email
echo "$USERNAME1" > /etc/mwstore/github/username
echo "$API1" > /etc/mwstore/github/api
echo "ON" > /etc/mwstore/github/gitstat
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
function resetipvps(){
clear
rm -f /etc/mwstore/github/email
rm -f /etc/mwstore/github/username
rm -f /etc/mwstore/github/api
rm -f /etc/mwstore/github/gitstat
echo "OFF" > /etc/mwstore/github/gitstat
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
Isadmin=$(curl -sS https://raw.githubusercontent.com/MyMasWayVPN/acces/main/punten | grep $MYIP | awk '{print $5}')
if [ "$Isadmin" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
GITREQ=/etc/mwstore/github/gitstat
if [ -f "$GITREQ" ]; then
    cekk="ok"
else 
    mkdir /etc/mwstore/github
    touch /etc/mwstore/github/gitstat
    echo "OFF" > /etc/mwstore/github/gitstat
fi

stst1=$(cat /etc/mwstore/github/gitstat)
if [ "$stst1" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1│${NC} ${COLBG1}               • REGISTER IPVPS •              ${NC} $COLOR1│$NC"
...
stst=$(cat /etc/mwstore/github/gitstat)
if [ "$stst" = "ON" ]; then
APIOK="CEK API"
rex="viewapi"
else
